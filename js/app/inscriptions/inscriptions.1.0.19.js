...
window.onload = async function () {
  ...
  try {
    ...
    alert(
      "To make sure inscribing will work properly, disable all adblockers for this app. If you use brave, turn off its shield." +
        "\n\n" +
...
async function showBackupUsage() {
  if ($("#backup-list").style.display == "none") {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      html +=
        '<div style="font-size: 14px;" id="backup-item-' +
        keys[i] +
        '">[<a href="javascript:void(0);" onclick="startInscriptionRecovery(\'' +
        keys[i] +
        '\')" style="font-size: 14px;">recover</a>] [<a href="javascript:void(0);" onclick="deleteInscription(\'' +
        keys[i] +
...
async function deleteInscription(key) {
  let result = confirm(
    "Are you sure you want to delete this backup entry? If you are not sure, check if there is anything to recover first."
  );
...
async function startInscriptionRecovery(key) {
  $("#backup-recovery").innerHTML =
    '<div id="recovery-info">Please wait, searching for UTXOs<span class="dots">.</span></div>';
  ...
  for (let i = 0; i < tx.length; i++) {
    ...
    let response = await getData(
      "https://mempool.space/" +
        mempoolNetwork +
        "api/address/" +
        Address.p2tr.encode(plainTapKey, encodedAddressPrefix) +
        "/utxo"
    );
    ...
    for (let j = 0; j < utxos.length; j++) {
      ...
      if (utxo !== null) {
        ...
        $("#backup-recovery").innerHTML +=
          '<div id="recovery-item-' +
          key +
          "-" +
          utxo.vout +
          '" style="font-size: 14px;">Found UTXO with ' +
          utxo.value +
          ' sats [<a style="font-size: 14px;" href="javascript:void(0);" onclick="recover(' +
          i +
          ", " +
          utxo.vout +
          ", $('#taproot_address').value, '" +
          key +
          "')\">recover</a>]</div>";
...
function showBrc20Transfer() {
  ...
  $("#brc-transfer-container").innerHTML =
    '<div class="brc-transfer-block">' + $$(".brc-transfer-block")[0].innerHTML + "</div>";
...
async function run(estimate) {
  ...
  if ($(".brc20_deploy_form").style.display != "none") {
    ...
    files.push({
      text: JSON.stringify(deploy),
      name: deploy.tick,
      hex: textToHex(JSON.stringify(deploy)),
      mimetype: mimetype,
      sha256: "",
    });
  ...
  if ($(".brc20_transfer_form").style.display != "none") {
    ...
    for (let i = 0; i < transfers.length; i++) {
      ...
      let mimetype = "text/plain;charset=utf-8";
      files.push({
        text: JSON.stringify(transfer),
        name: transfer.tick,
        hex: textToHex(JSON.stringify(transfer)),
        mimetype: mimetype,
        sha256: "",
      });
    }
  ...
  if (active_plugin === null) {
    if ($(".file_form").style.display == "block") {
      if (!estimate && tip_check < 500 * files.length) {
        ...
        alert(
          "Minimum tipping is " +
            500 * files.length +
            " sats based on your bulk amount. A suggestion has been added to the tip."
        );
        return;
      }
    } else {
    ...
    if (!estimate && tip_check < plugin_tip) {
      ...
      $("#tip-usd").innerHTML = Number(await satsToDollars($("#tip").value)).toFixed(2);
      alert(
        "Minimum tipping has been set to " +
          plugin_tip +
          " sats based on your inscriptions. A suggestion has been added to the tip."
      );
      return;
    }
  }
  ...
  if (estimate) {
    $("#estimated-fees").innerHTML =
      " = " + total_fees + " sats ($" + Number(await satsToDollars(total_fees)).toFixed(2) + ")";
    return files;
  }
  ...
  $(".display").innerHTML +=
    "<p>" +
    padding * inscriptions.length +
    ` sats will go to the address.</p><p>${total_fee} sats will go to miners as a mining fee.</p><p>${overhead} sats overhead will be used as boost.</p>`;
  ...
  $(".modal-content").innerHTML =
    '<div id="funds-msg">Inscriptions about to begin. Please wait' +
    (!$("#cpfp").checked ? " for the seed transaction to confirm (CPFP disabled)" : "") +
    "...</div>";
  $(".modal").style.display = "block";
...
async function recover(index, utxo_vout, to, privkey) {
  if (!isValidTaprootAddress(to)) {
    $("#recovery-item-" + privkey + "-" + utxo_vout).innerHTML +=
      '<div style="font-size: 14px;">Invalid taproot address. Please add the recovery recipient in the "Receiving address" at the very top.</a>';
  ...
  if (utxo.value - base_fee <= 0) {
    $("#recovery-item-" + privkey + "-" + utxo_vout).innerHTML +=
      '<div style="font-size: 14px;">Nothing found to recover: ' + (utxo.value - base_fee) + " sats</div>";
  ...
  $("#recovery-item-" + privkey + "-" + utxo_vout).innerHTML +=
    '<div style="font-size: 14px;">Result: ' + _txid + "</div>";
}
...
async function init(num) {
  ...
  let isgreen;
  $$(".fee .num").forEach(function (item) {
    if (item.style.backgroundColor == "green" || getComputedStyle(item).backgroundColor == "rgb(0, 128, 0)")
      isgreen = item;
  });
  if (isgreen) {
